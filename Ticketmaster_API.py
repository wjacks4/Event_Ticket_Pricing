...
def Data_Fetch_pymysql():
	...
	Artists_DF = pd.read_sql('SELECT * FROM Artists_trimmed_ranked WHERE current_followers >= 20000 and current_followers <= 500000 ', con = connection)
...
#def EVENT_DETAILS():
	...
	#Artists_df = Data_Fetch_pymysql().head(100)
	...
	for IDs_dat in IDs_df.iterrows():
		...
		if j <= 10:
			...
			api_key = ('.json?apikey=83sdXVyv4k3NnuGCIvk5nAHE3NSWddg7')
			event_url = (event_base_url + event_id + api_key1)
			...
			try:
				...
				try:
					event_state_full = json_Dat['_embedded']['venues'][0]['state']['name']
			...
			api_key = ('.json?apikey=2C4llrNfIrGgEZxAft1QuJ5bpbS3SdpF')
